...
export async function buildOurSimpleTransaction({
  connection,
  makeTxVersion,
  payer,
  innerTransactions,
  ...
  recentBlockhash?: string | undefined
  addLookupTableInfo?: CacheLTA | undefined
}, inputToken: PublicKey, inputTokenAmount: number): Promise<any> {
  if (makeTxVersion !== TxVersion.V0 && makeTxVersion !== TxVersion.LEGACY) throw Error(' make tx version args error')
  ...
  if (inputToken.toBase58() == 'So11111111111111111111111111111111111111112') {
    ...
    instructions.push(createAssociatedTokenAccountIdempotentInstruction(
      payer,
      getAssociatedTokenAddressSync(new PublicKey("So11111111111111111111111111111111111111112"), payer),
      payer,
      inputToken,
    ...
    instructions.push(
      SystemProgram.transfer({
        fromPubkey: payer,
        toPubkey: getAssociatedTokenAddressSync(new PublicKey("So11111111111111111111111111111111111111112"), payer),
  ...
  txList.push(
    _makeTransaction({
      ...
      lookupTableInfos: Object.values({
        ...(addLookupTableInfo ?? {}),
        ...((innerTransactions[0] as InnerSimpleV0Transaction).lookupTableAddress ?? {}),
      }),
    }),
  )
  // }
