...
function formatAmmKeysById(id, cm) {
    return __awaiter(this, void 0, void 0, function* () {
        const account = yield cm.connSync({ changeConn: true }).getAccountInfo(new web3_js_1.PublicKey(id));
        if (account === null)
            throw Error(' get id info error ');
        const info = raydium_sdk_1.LIQUIDITY_STATE_LAYOUT_V4.decode(account.data);
        const marketId = info.marketId;
        //console.log('wat : ' + marketId);
        const marketAccount = yield cm.connSync({ changeConn: true }).getAccountInfo(marketId);
        ...
        const lpMintAccount = yield cm.connSync({ changeConn: true }).getAccountInfo(lpMint);
